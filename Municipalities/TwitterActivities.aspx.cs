...
{
    public partial class TwitterActivities : System.Web.UI.Page
    {
        ...
        protected void FillTw()
        {


            model = new municipalitiesEntities();
        
            twitter = new ObservableCollection<TwitterNew>(model.TwitterNews.OrderBy(a => a.TDay & a.TMonth & a.TYear));
            pd.DataSource = twitter;
            rptTw.DataSource = pd;
            pd.AllowPaging = true;
            pd.PageSize = 15;
        ...
        private void doPaging()
        {
            ...
            if (CurrentPage > 5)
            {
                lindex = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(twitter.Count / pd.PageSize)));
            }
            else
            {
                lindex = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(twitter.Count / pd.PageSize)));
