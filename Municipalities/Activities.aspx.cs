...
namespace Municipalities
{
    public partial class Activities : System.Web.UI.Page
    {
       ...
       // private int pos;
        ...
        protected void FillFB()
        {
           
           
            model = new municipalitiesEntities();
         
            facebook = new ObservableCollection<FBNew>(model.FBNews.OrderBy(a => a.FBday & a.FBmonth & a.FByear));
         



            pd.DataSource = facebook;
            rptFB.DataSource = pd;
            pd.AllowPaging = true;
            pd.PageSize = 8;
        ...
        private void doPaging()
        {
            ...
            if (CurrentPage > 5)
            {
                lindex = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(facebook.Count / pd.PageSize)));
            }
            else
            {
                lindex = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(facebook.Count / pd.PageSize)));
